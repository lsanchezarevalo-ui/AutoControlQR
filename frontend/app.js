...
async function req(url,opt={}){const headers={...(opt.headers||{})};if(token())headers.Authorization='Bearer '+token();try{const r=await fetch(url,{...opt,headers,cache:'no-store'});let j={};try{j=await r.json()}catch{}return{r,j,networkError:null}}catch(e){return{r:null,j:{},networkError:e}}}
...
function openFormModal({title,body,onSubmit}){
 ...
 wrap.innerHTML=`<div class="modalcard"><div class="modalhead"><h2>${esc(title)}</h2><button type="button" class="iconbtn closemodal">×</button></div><form class="modalform">${body}<div class="modalactions"><button type="button" class="secondary closemodal">Cancelar</button><button type="submit">Guardar</button></div><p class="error modalerr"></p></form></div>`;
...
async function assignPlan(vehicleId){const sel=document.getElementById('plan-'+vehicleId);if(!sel.value)return alert('Selecciona un plan.');const out=await req(`${api}/vehicles/${vehicleId}/assign-plan`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({planVersionId:sel.value})});if(!out.r||!out.r.ok)return alert('No se pudo asignar el plan.');adminShell('vehicles')}
...
async function registerMaintenance(vehicleId,currentMileage,serviceId,serviceName){
 const wrap=openFormModal({
  ...
  onSubmit:async(f,err)=>{
   ...
   const out=await req(`${api}/vehicles/${vehicleId}/maintenance`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({mileage:km,serviceDate:date,serviceIds:[serviceId],notes})});
