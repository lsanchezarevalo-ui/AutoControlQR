...
function vehicleCreateFormFields(catalog,plans){
 return `<label>Placa<input name="plate" class="force-upper" placeholder="ABC-123" autocomplete="off" required></label><label>Número interno<input name="internal" class="force-upper" placeholder="Opcional" autocomplete="off"></label>${catalogFields(catalog,'vehicle')}<label>Kilometraje<input name="km" class="integer-km" type="text" inputmode="numeric" pattern="[0-9]*" placeholder="134500" required></label><label>Plan de mantenimiento <small class="optional-label">Opcional</small><select name="planVersionId"><option value="">Sin plan por ahora</option>${plans.map(p=>`<option value="${p.versionId}">${esc(p.name)} V${p.versionNumber}</option>`).join('')}</select></label><div class="vehicle-create-action"><button>Crear vehículo</button></div>`;
}
function wireVehicleCreateForm(catalog){
...
function renderVehicles(vehicles,plans,isAdmin=true,action=null){
 ...
 const mobileMode=(isAdmin&&isMobile&&action)?action:null;
 const backLink=mobileMode?`<button type="button" id="vehiclesBackMenu" class="textbtn quick-menu-back">‹ Volver</button>`:'';

 if(mobileMode==='create'){
  main.innerHTML=backLink+card(`${moduleTitle('vehicle','Nuevo vehículo','Completa los datos. El plan es opcional; también puedes crear servicios propios para este vehículo.')}<form id="create" class="vehicle-create-grid">${vehicleCreateFormFields(catalog,plans)}</form><p id="err" class="error"></p>`);
  wireVehicleCreateForm(catalog);
 ...
 if(mobileMode==='search'||mobileMode==='list'){
  main.innerHTML=backLink+card(isAdmin?`<div class="top vehicle-page-head">${moduleTitle('vehicle','Vehículos','Administra la flota y consulta el mantenimiento de cada unidad.')}<button type="button" id="showArchivedVehicles" class="secondary mobile-archived-btn">Ver archivados</button></div>`:`<h1>Vehículos</h1><p class="muted">Consulta el estado y registra los mantenimientos realizados.</p>`)+listHeadHtml+listHtml;
  if(isAdmin)document.getElementById('showArchivedVehicles').onclick=()=>showArchivedVehicles(plans);
  const vehicleSearch=document.getElementById('vehicleSearch');
  vehicleSearch.oninput=()=>{const q=vehicleSearch.value.trim().toUpperCase();const filtered=!q?vehicles:vehicles.filter(v=>String(v.plate||'').toUpperCase().includes(q)||String(v.internalNumber||'').toUpperCase().includes(q));document.getElementById('vehicleList').innerHTML=filtered.length?filtered.map(v=>vehicleCard(v,plans,isAdmin)).join(''):'<div class="no-search-results">No encontramos vehículos con esa placa o número interno.</div>';document.getElementById('vehicleCount').textContent=filtered.length;wireVehicleButtons(filtered,plans,isAdmin)};
  wireVehicleButtons(vehicles,plans,isAdmin);
  document.getElementById('vehiclesBackMenu').onclick=()=>adminShell('vehicles');
  if(mobileMode==='search')vehicleSearch.focus();
  return;
 }

 main.innerHTML=(isAdmin?card(`<div class="top vehicle-page-head">${moduleTitle('vehicle','Vehículos','Administra la flota y consulta el mantenimiento de cada unidad.')}<button type="button" id="showArchivedVehicles" class="secondary mobile-archived-btn">Ver archivados</button></div><div class="vehicle-create-title"><h2>Nuevo vehículo</h2><p class="muted">Completa los datos. El plan es opcional; también puedes crear servicios propios para este vehículo.</p></div><form id="create" class="vehicle-create-grid">${vehicleCreateFormFields(catalog,plans)}</form><p id="err" class="error"></p>`):card(`<h1>Vehículos</h1><p class="muted">Consulta el estado y registra los mantenimientos realizados.</p>`))+listHeadHtml+listHtml;
 const vehicleSearch=document.getElementById('vehicleSearch');
 vehicleSearch.oninput=()=>{const q=vehicleSearch.value.trim().toUpperCase();const filtered=!q?vehicles:vehicles.filter(v=>String(v.plate||'').toUpperCase().includes(q)||String(v.internalNumber||'').toUpperCase().includes(q));document.getElementById('vehicleList').innerHTML=filtered.length?filtered.map(v=>vehicleCard(v,plans,isAdmin)).join(''):'<div class="no-search-results">No encontramos vehículos con esa placa o número interno.</div>';document.getElementById('vehicleCount').textContent=filtered.length;wireVehicleButtons(filtered,plans,isAdmin)};
 if(isAdmin){
