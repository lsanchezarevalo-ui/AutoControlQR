async function adminShell(view='dashboard',action=null){
 ...
 if(view==='dashboard')renderDashboard(vehicles);else if(view==='plans'){if(isMobile&&!action)renderPlansQuickMenu();else renderPlans(plans,vehicles,action)}else if(view==='reports')renderReports(vehicles);else if(view==='users')renderUsers();else if(view==='notifications')renderNotifications();else if(view==='settings')renderCompanySettings();else{if(isMobile&&!action)renderVehiclesQuickMenu();else renderVehicles(vehicles,plans,true,action)}
}
