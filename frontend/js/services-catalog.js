...
async function renderServicesCatalog(){
 ...
 const wire=()=>{
  document.querySelectorAll('.editcs').forEach(b=>b.onclick=()=>editServiceCatalog(services.find(x=>x.id===b.dataset.id)));
...
function editServiceCatalog(s){
 if(!s)return;
 const wrap=openFormModal({
  title:`Editar servicio · ${s.name}`,
  body:serviceCatalogFormFields({name:s.name,category:s.category,spec:s.specification||'',intervalKm:s.defaultIntervalKm||'',intervalMonths:s.defaultIntervalMonths||'',pre:s.defaultPrealertKm||0}),
