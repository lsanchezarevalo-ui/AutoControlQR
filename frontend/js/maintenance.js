async function maintenanceView(vehicleId){
 ...
 document.getElementById('main').innerHTML=card(`<button id="back" class="secondary">← Volver</button><div class="maintenance-page-head"><div><h1>${plateLine}</h1><p><b>${fmt(d.currentMileage)} km</b> ${d.overallStatus==='NO_PLAN'?'<span class="badge mutedb">Servicios individuales</span>':statusBadge(d.overallStatus)} ${d.hasIncompleteHistory?'<span class="badge mutedb">⚫ Historial incompleto</span>':''}</p></div><h2>Estado de mantenimiento</h2>${isAdmin && (d.individualServices||d.overallStatus==='NO_PLAN')?'<button id="addVehicleService">+ Agregar servicio</button>':''}</div>${rows||'<div class="no-search-results"><b>Aún no hay servicios</b><small>Agrega solamente los mantenimientos que deseas controlar para este vehículo.</small></div>'}`);
