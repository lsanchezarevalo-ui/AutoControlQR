...
public static class UserEndpoints
{
    public static void MapUserEndpoints(this WebApplication app, string connectionString)
    {
        ...
        app.MapGet("/api/v1/users", async (ClaimsPrincipal principal) =>
        {
            ...
            await using var cmd=new NpgsqlCommand("SELECT id,full_name,email,role,status,created_at FROM users WHERE company_id=@c AND role<>'PLATFORM_ADMIN' ORDER BY full_name",con);cmd.Parameters.AddWithValue("c",companyId);
        ...
        app.MapPatch("/api/v1/users/{id:guid}/status", async (ClaimsPrincipal principal, Guid id, UserStatusRequest req) =>
        {
            ...
            await using var con=new NpgsqlConnection(connectionString);await con.OpenAsync();await using var cmd=new NpgsqlCommand("UPDATE users SET status=@s WHERE id=@id AND company_id=@c AND role<>'PLATFORM_ADMIN'",con);
        ...
        app.MapPatch("/api/v1/users/{id:guid}/password", async (ClaimsPrincipal principal,Guid id,ResetUserPasswordRequest req) =>
        {
         ...
         await using var con=new NpgsqlConnection(connectionString);await con.OpenAsync();
         DemoUser? user=null;await using(var q=new NpgsqlCommand("SELECT id,company_id,full_name,email,role FROM users WHERE id=@id AND company_id=@c AND role<>'PLATFORM_ADMIN'",con)){q.Parameters.AddWithValue("id",id);q.Parameters.AddWithValue("c",CompanyId(principal));await using var r=await q.ExecuteReaderAsync();if(await r.ReadAsync())user=new DemoUser(r.GetGuid(0),r.GetGuid(1),r.GetString(2),r.GetString(3),"",r.GetString(4));}
